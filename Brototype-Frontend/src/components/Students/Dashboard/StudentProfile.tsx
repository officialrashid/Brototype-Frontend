  ...
  const fetchStudentProfile = async () => {
    ...
    return (
            ...
            <div className="w-72 h-36 absolute top-3 left-32">
              <h1 className="ml-24 text-xl font-semibold mt-10 font-roboto">{profileInfo?.firstName ?? "Muhammed"}  {profileInfo?.lastName ?? "Rashid k"}</h1>
              <h4 className="ml-28 mt-1 text-gray-500 mt-2 text-sm font-roboto">{profileInfo?.domain ?? "eg:-Mernstack developer"}</h4>
            ...
            <div className="border border-2px w-28 h-28 m-8 shadow-xl rounded-md absolute top-3 right-3">
              <img src={profileInfo?.imageUrl ?? "/profile.jpeg"} alt="" className="w-full h-full rounded-md" />
            </div>
            </div>
