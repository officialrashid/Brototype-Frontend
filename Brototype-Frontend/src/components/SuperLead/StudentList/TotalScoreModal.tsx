...
const TotalScoreModal = ({ isVisible, isClose, week, studentId, batchId }) => {
        ...
        const fetchPendingTopics = async () => {
            try {
                ...
                if (response) {
                    ...
                    const data = {
                        reviewScore: weekData.reviewScore,
                        personalWorkoutsScore: weekData.personalWorkoutsScore,
                        miscellaneousWorkouts: weekData.miscellaneousWorkouts,
                        communicationScore: weekData.communicationScore,
                        totalScore: weekData.reviewScore+weekData.personalWorkoutsScore+weekData.miscellaneousWorkouts+weekData.communicationScore
                    }
                    if (weekData) {
                        console.log(weekData);
            ...
            <div className="fixed inset-0 bg-opacity-20 backdrop-blur-sm flex justify-center items-center overflow-y-scroll overflow-hidden z-40">
                <div className="border border-gray-200 m-5 rounded-lg shadow-2xl w-2/5 bg-white">
                    ...
                    <div className="text-center">
                        <div><span className="font-semibold font-roboto text-md mb-2">Score Sheet</span></div>
                    </div>
                    <div className="m-5 mt-6">
                        <div className="">
                            <textarea
                                cols={30}
                                rows={10}
                                value={`Review Score :- ${totalScore.reviewScore}\npersonal Workouts Score :- ${totalScore.personalWorkoutsScore}\nmiscellaneous Workouts Score :- ${totalScore.miscellaneousWorkouts}\ncommunication Score :- ${totalScore.communicationScore}\nTotal Score :- ${totalScore.totalScore}`}
                                readOnly
